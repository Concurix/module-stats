...
Archiver.prototype.start = function start() {
  ...
  this.interval = setInterval(this.send.bind(this), this.options.archiveInterval);
}
...
Archiver.prototype.computePackage = function computePackage() {
  ...
  for (var i = 0; i < linkKeys.length; i++) {
    ...
    if(links[key].type == 'callback'){
      console.log('link event delay is ', links[key].num_calls && links[key].event_loop_delay / links[key].num_calls, links[key].id);
    }
  }
