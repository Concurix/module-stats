...
function doArchive(json) {
  console.log('got doArchive of ', s3Handler);
  if (!s3Handler){
    initAWS.push (json);
    return;
  };
...
function configure(options){
  config = options; 
  if( !options.archiveInterval ){
    console.log("yikes no archiveInterval");
    console.trace();
  }
  console.log('config with ',options );
...
function tickle() {
  if( archiveTickled ){
    ...
    console.log('setup timer for ', config.archiveInterval);
    if(!config.archiveInterval){
      console.trace();
    }
