...
function acquireAwsCredentials() {
  gettingCredentials = true;  // note that we will never reset this to false once we've started an init process
  ...
  function start(str) {
    ...
    // TODO does this keep from immediate stop when cx.stop() called?
  ...
  initAWS = [];
  var stream = require('http').request(options, requestCallback);
  stream.on('error', function(err){
    gettingCredentials = false; 
    emitter.emit('archiveOffline', err);
  });
  stream.end();
}

function doArchive(packet) {
  if (skipArchive) {
    return;
  }
  ...
  var key      = s3KeyPrefix + '/' + process.pid + '/' + unixSec + '.json';

  // Write the data to S3
  aggregate.stopTracing();
  var body = JSON.stringify(packet);

  //once we have pickled our data, go ahead and clear the aggregate cache.
  global.concurix.traceAggregate.resetTimes();

  s3Handler.putObject({Bucket: s3BucketName, Key: key, Body: body}, function(putErr, putData) {
    if (!putErr) {
      ...
      snsHandler.client.publish(params, function(err, data) {
        ...
        emitter.emit('sendArchive', packet);
      });
...
function configure(options){
  ...
  if( options.archiveOffline ){
    emitter.on('archiveOffline', options.archiveOffline);
  }
  ...
  if (options.accountKey && options.hostname && options.archiveHost && options.archivePort && !skipArchive) {
    initAwsCredentials();
  }
}
...
function computePackage() {
  //quick validation
  var links = global.concurix.traceAggregate.linkCache;
  var nodes = global.concurix.traceAggregate.nodeCache;
  for( key in links){
  ...
  var msg = {
    type: "nodejs",
    version: version,
    tracing_interval: config.archiveInterval,
    hostname: HOSTNAME,
    // run_id: 'to be set',
    ...
    data: {
      ...
      links:   util.values(global.concurix.traceAggregate.linkCache)
    }
  };
  return msg;
}
