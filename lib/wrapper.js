...
function wrapReturn(trace, clientState, hooks, rules){
  if( wrap.isWrapper(util)){
    console.log('util got wrapped');
  }
  if( util.isFunction(trace.ret) || util.isObject(trace.ret)){
    module.exports.wrapFunctions(trace.moduleName, trace.ret, 0, hooks, clientState, rules );
  }
}
