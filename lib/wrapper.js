...
function Wrapper(aggregator, options) {
  ...
  var rules = new Rules(this.options.rules);
  this.rules = rules;
  var hooks = {
    beforeHook: function beforeHook(trace, clientState) {
      ...
      aggregator.handleBeforeTrace(trace, clientState);
    ...
    afterHook: function afterHook(trace, clientState) {
      ...
      aggregator.handleAfterTrace(trace, clientState);
    }
  }
  this.hooks = hooks;
}
...
Wrapper.prototype.wrapExports = function wrapExports(name, obj){
  ...
  var state = {
    modInfo: {
      top: (this.options.moduleTop) || name,
      requireId: name,
      id: (this.options.moduleId) || name
    },
    rules: this.rules
  };
  ...
  if (this.options.state) {
    state.clientState = this.options.state;
  }
  //wrap the functions first
  wrapFunctions(name, obj, -1, this.hooks, state, this.rules);
  //check to see if the  object itself is a function, if so, wrap it
  if (util.isFunction(obj) && !wrap.isWrapper(obj) && this.rules.wrapRequireReturn()) {
    ret = wrap(obj)
          .before(this.hooks.beforeHook)
          .after(this.hooks.afterHook)
          .state(state)
          .nameIfNeeded(name)
          .module(name)
          .getProxy();
