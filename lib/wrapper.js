...
Wrapper.prototype.generateRules = function generateRules(override) {
  ...
  return new Rules(this.optionsOverride(override).rules);
}
...
Wrapper.prototype.generateHooks = function generateHooks(override) {
  ...
  var hooks = {
    beforeHook: function beforeHook(trace, clientState) {
      trace.wrapperStart = process.hrtime();
    ...
    afterHook: function afterHook(trace, clientState) {
      ...
      var depth = self.aggregator.handleAfterTrace(trace, clientState);
      // IMPORTANT: Do not do anything expensive after here as it will NOT
      //   be part of the time accounting!

      // NOTE: This check intentionally commented out because
      //   of various issues with the entire flow of callbackOf
      //   due to the wrapper's singleton nature. For now we're
      //   accepting the identity of the wrapper being the last
      //   thing that called it as a callback.
      // if (clientState.callbackOf != null) {
      //   // What about the stream or EventEmitter case?
      //   clientState.callbackOf = undefined;
      // }
      //console.log(trace);
      //console.log("%s-- fnTime: %s totalTime: %s childFnTime: %s wrapperCost: %s childTotalTime: %s oldDuration %s", trace.funInfo.name, trace.fnTime, trace.wrapperEndOffset, trace.childFnTime, trace.wrapperCost, trace.childTotalTime, util.hrtToUs(trace.execTime))
      // if (depth === 0) {
      //   console.log("ZERO'D")
      // }
    }
