...
exports.handleBeforeTrace = function handleBeforeTrace(trace, clientState){
  ...
  if( stack ){
    ...
    trace.transcationId = trace.calledBy ? trace.calledBy.transactionId : null;
  }
  if(clientState.callbackOf){
    trace.callbackOf = clientState.callbackOf;
    trace.transcationId = clientState.callbackOf.transactionId;
  }

  if( trace.transactionId ){
    trace.name += trace.transactionId;
  }
