...
Aggregator.prototype.handleAfterTrace = function handleAfterTrace (trace, clientState) {
  ...
  trace.fnStopOffset = util.diffUs(trace.fnEnd, trace.wrapperStart);
  trace.fnTime = trace.fnStopOffset - trace.fnStartOffset - trace.childTotalTime;

  if (caller) {
    // TODO shaped object
    if (!caller.childTotalTime) {
      caller.childTotalTime = trace.childTotalTime;
    }
