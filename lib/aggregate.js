...
Aggregator.prototype.reset = function reset() {
  ...
  this.ageNodes = {};
  this.traceStack = [];
  this.traceStack.eventLoopCounter = 0;
}
...
Aggregator.prototype.handleBeforeTrace = function handleBeforeTrace(trace, clientState) {
  ...
  if (this.traceStack) {
    ...
    trace.eventLoopCounter = this.traceStack.eventLoopCounter;
  }
...
Aggregator.prototype.handleAfterTrace = function handleAfterTrace (trace, clientState) {
  ...
  if (linkCache) {
    ...
    links.forEach(function(link){
      ...
      if (link.type == 'callback') {
        ...
        trace.callbackOf.eventLoopCounter = trace.eventLoopCounter;
      }
  ...
  if (this.traceStack) {
    ...
    if( this.traceStack.length == 0 ) {
      this.traceStack.eventLoopCounter++;
    }
  }
