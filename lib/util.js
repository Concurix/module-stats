...
Util.isFunction = function isFunction(obj) {
  ...
  return funcCheck && !primitiveConstructor && !Util.isPrimitiveType(obj);
};
...
Util.subHrtTimes = function subHrtTimes(t1, t2){
  return [t1[0] - t2[0], t1[1] - t2[1]];
}
...
Util.diffUs = function diffUs(end, start) {
  return Util.hrtToUs(end) - Util.hrtToUs(start);
}
