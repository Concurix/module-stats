...
exports.store = function store(trace, clientState) {
  var node = {
    ...
    merge: function merge(trace){
      ...
      this.num_calls += 1;
      this.duration += ((trace.childFnTime || 0) + (trace.fnTime || 0));
      this.child_duration += trace.childFnTime || 0;

      this.fnTime += trace.fnTime || 0;
      this.childFnTime += trace.childFnTime || 0;
      this.childTotalTime += trace.childTotalTime || 0;
      this.wrapperCost += trace.wrapperCost || 0;
      this.fnStartOffset += trace.fnStartOffset || 0;
      this.fnEndOffset += trace.fnEndOffset || 0;
      this.wrapperEndOffset += trace.wrapperEndOffset || 0;
    }
  };
