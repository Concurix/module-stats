...
exports.store = function store(trace, clientState) {
  if( clientState && clientState.modInfo.top == 'express'){
  ...
  var node = {
    ...
    pid: trace.processId,
    fun_name: trace.transactionId ? trace.funInfo.name + "for " + trace.transactionId: trace.funInfo.name,
...
function traceName(trace){
  ...
  if( trace.transactionId ) {
    name = [trace.processId, trace.name];
  } else {
    name = [trace.processId, trace.name, trace.transactionId];
  }
  return name.join(':');
}
